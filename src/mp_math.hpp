...
constexpr T Pow(T x, int y) {
  if (y == 0) {
    ...
    T mult_val = y_is_negative ? (1/x) : x;
...
// estimates cox(x) using a taylor series expansion
// https://en.wikipedia.org/wiki/Taylor_series
constexpr double Cos(double x, unsigned taylor_terms=32) {
  double factorial = 1.0;
  double power = 1.0;
  double answer = 1.0;
  ...
  for(int i = 1; i < taylor_terms-1; i++) {
    power *= x * x;
    factorial *= factorial_idx++;
    factorial *= factorial_idx++;
    answer += (negative_sign) ? (-power / factorial) : (power / factorial);
    negative_sign = !negative_sign;
  }
  return answer;
}

// estimates sin(x) using a taylor series expansion
// https://en.wikipedia.org/wiki/Taylor_series
constexpr double Sin(double x, unsigned taylor_terms=32) {
  double factorial = 1.0;
  double power = x;
  double answer = x;
  bool negative_sign = true;
